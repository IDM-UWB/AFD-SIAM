function subidx = theta2realidx(x,xqtie)
% THETA2REALIDX returns real subindices of grid cell for vectors
%   subidx = THETA2REALIDX(x,xqtie) returns subindices 'subidx' of a nD
...
for i = 1:sizX(1)
   ...
   subidx(i,:) = 1+(x(i,:)-theta_min)/delta_theta;
   subidx(i,:) = max(min(subidx(i,:),imax),imin);
end
